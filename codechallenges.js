...
function sayHello() {
  ...
  function isPalindrome(str) {
    const strReverse = str.toLowerCase().split('').reverse()
    for (idx = 0; idx < strReverse.length; idx ++){
      if (strReverse[idx] === ' '){
        strReverse.splice(idx, 1)
      }
    } const strReverse2 = strReverse.join('');
    const mainStr = str.toLowerCase().split('')
    for (idx = 0; idx < mainStr.length; idx ++){
      if (mainStr[idx] === ' '){
        mainStr.splice(idx, 1)
      }
    } const mainStr2 = mainStr.join('');
    return mainStr2 === strReverse2 ? true : false;
  }
  ...
  function mumble(str){
    ...
    for (idx = 0; idx < chars.length; idx++){
      if (idx != 0) output += '-';
      let line = chars[idx].repeat(idx+1)
      output += line.charAt(0) + line.substring(1)
    } return output
  }
